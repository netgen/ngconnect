    ...
    <span id="ngconnect">
        {foreach $login_methods as $l}
            {if or( $is_user_anonymous, and( $is_user_anonymous|not, fetch( ngconnect, user_has_connection, hash( user_id, $user.contentobject_id, login_method, $l ) )|not ) )}
                ...
                {else}
                    <a href={concat( 'ngconnect/login/', $l, '?redirectURI=', cond( and( is_set( $module_result.node_id ), is_set( $module_result.content_info.url_alias ) ), $module_result.content_info.url_alias, true(), '' )|urlencode )|ezurl}><img src={concat( 'ngconnect/', $l, '.png' )|ezimage} alt="{$method_name|wash}" /></a>
                {/if}
            {/if}
        {/foreach}
    </span>
